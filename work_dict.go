...
func StoreDictionary(d Dictionary) {
	...
	err1 := ioutil.WriteFile("dict.opcorpora.xml.gob", network.Bytes(), 0777) // just pass the file name
	if err1 != nil {
		fmt.Print(err1)
	}
}
...
func LoadDictionary() Dictionary {
	b, err := ioutil.ReadFile("dict.opcorpora.xml.gob") // just pass the file name
	if err != nil {
		fmt.Print(err)
	}
...
func StoreDictionaryOC(d *DictionaryOC) {
	...
	err1 := ioutil.WriteFile("dict.opcorpora.json.gob", network.Bytes(), 0777) // just pass the file name
	if err1 != nil {
		fmt.Print(err1)
	}
}
...
func LoadDictionaryOC() *DictionaryOC {
	b, err := ioutil.ReadFile("dict.opcorpora.json.gob") // just pass the file name
	if err != nil {
		fmt.Print(err)
	}
	bb := bytes.NewBuffer(b)
...
func FindDictionaryOC(d *DictionaryOC, link_type string) ([]string, error) {
	...
	for _, v := range d.Links {
		...
		if v.Type == link_type {
			...
			if !ok1 {
				return res, errors.New("From not foud lemmas")
			}
			lt, ok2 := d.Lemmas[v.To]
			if !ok2 {
				return res, errors.New("To not foud lemmas")
			}
...
func FindLinkToFromDictionaryOC(d *DictionaryOC, link_type string, to string) (string, error) {
	...
	if ok {
		lf, ok1 := d.Lemmas[id]
		if ok1 {
			return lf.Base, errors.New("From not foud lemmas")
		}
	}
	return res, nil
}
...
func StoreCorpora(d *Corpora) {
	...
	err1 := ioutil.WriteFile("opcorpora.gob", network.Bytes(), 0777) // just pass the file name
	if err1 != nil {
		fmt.Print(err1)
	}
}
...
func LoadCorpora(file_name string) *Corpora {
	b, err := ioutil.ReadFile(file_name) // "opcorpora.gob" just pass the file name
	if err != nil {
		fmt.Print(err)
	}
	bb := bytes.NewBuffer(b)
...
func SaveSettings(file_name string, settings *Settings) error {
	...
	_ = ioutil.WriteFile(file_name, data, 0644)
	return nil
}

func LoadBadWordsList() ([]string, error) {
	rsl := []string{}
	b, err := ioutil.ReadFile("bad_words.txt")
	if err != nil {
		fmt.Print(err)
		return nil, err
	}
...
func OpenOCorpora(path string) (*OCorpora, error) {
	psdb := Init_SmallDB(path) // "./OCorporaDB"
	sdb := &psdb
	sdb.Debug = 0
	if !sdb.Inited {
		fl := []string{"word_form", "word", "word_property"}
		res := sdb.CreateDB(fl, path) //  "./OCorporaDB"
		if res != 0 {
...
func OpenOCorporaShort(path_db string) (*OCorpora, error) {
	psdb := Init_SmallDB(path_db) // "./OCorporaDBShort"
	sdb := &psdb
	sdb.Debug = 0
	if !sdb.Inited {
		fl := []string{"word_form", "word", "word_property"} // слово-форма, базовая форма, локальные свойства
		res := sdb.CreateDB(fl, path_db)                     // "./OCorporaDBShort"
		if res != 0 {
			fmt.Printf("Error creating DB %v\r\n", res)
			os.Exit(res)
		}
...
func (oc *OCorpora) StoreToSmallDB(co *Corpora) {
	for i, _ := range co.WordProp {
		...
		for k, _ := range wp.WordFormPropertiesDict {
			word_form_data := k
...
func (oc *OCorpora) StoreToSmallDBFull(co *Corpora, bad_words_list []string) {
	...
	for i, _ := range co.WordProp {
		...
		for k, _ := range wp.WordFormPropertiesDict {
			...
			for j, _ := range wpi {
				...
				props := []string{}
				flag_form := true
				for m, _ := range wpi[j].Properties {
				...
				if flag_form {
					for l, _ := range wpi_n {
						for n, _ := range wpi_n[l].Properties {
				...
				res1, num1, err1 := oc.Sdb.Store_record_on_map(args)
				if err1 != nil {
				...
				if false {
					if flag_bw {
						...
						ds, num, err := oc.Sdb.Find_record_index_string([]string{"word_form"}, wfdl)
	...
	err1 := ioutil.WriteFile("writed.json", prop, 0777)
	if err1 != nil {
		fmt.Println(err1)
		return
	}
...
func (oc *OCorpora) StoreToSmallDBShort(co *Corpora) {
	...
	for i, _ := range co.WordProp {
		...
		*/
		...
		oc.Sdb.Store_record("fake_"+Unique_Value(7), word_data, string(wp_data))
		for k, v := range wp.WordFormPropertiesDict {
			...
			oc.Sdb.Store_record(word_form_data, word_data+"_"+Unique_Value(7), string(local_data))
		}
		// проверяем
		wfdl := []string{word_data}
		ds, rec_id, err1 := oc.Sdb.Find_record_index_string([]string{"word"}, wfdl)
		if err1 != nil {
			fmt.Printf("word_data %v\r\n", word_data)
			fmt.Printf("ds %v num %v err %v\r\n", ds, rec_id, err1)
			...
			for k, _ := range wp.WordFormPropertiesDict {
				wfdl := []string{k}
				ds2, num, err2 := oc.Sdb.Find_record_index_string([]string{"word_form"}, wfdl)
				if err2 != nil {
	...
	for i, _ := range co.WordProp {
		...
		for k, _ := range wp.WordFormPropertiesDict {
			...
			for j, _ := range wpi {
				...
				// args["word"] = word_data
				flag := 0
				for n_rec, _ := range ds {
					rec := ds[n_rec]
					val, err := oc.Sdb.Get_field_value_by_name(rec, "word")
					if err != nil {
						...
						if val != word_data {
							...
							for m, _ := range wpi[j].Properties {
								...
								if attr == "" {
									...
									//fmt.Printf("attr %v")
									if attr != "POS" || attr != "animacy" || attr != "aspects" || attr != "cases" || attr != "genders" || attr != "involvement" || attr != "moods" || attr != "numbers" || attr != "persons" || attr != "tenses" || attr != "transitivity" || attr != "voices" {
									} else {
										val, err := oc.Sdb.Get_field_value_by_name(rec, attr)
										if err != nil {
		...
		if len(ds) > 0 {
			for k, _ := range ds {
				...
				if len(res) > 0 {
					...
					ds1, num1, err1 := oc.Sdb.Find_record_index_string([]string{"word"}, word_data1)
					if err1 != nil {
...
func FindWordCorporaFull(word string, oc *OCorpora) ([]ResultWordProp, error) {
	...
	if len(ds) > 0 {
		for i, _ := range ds {
			...
			wordform, err := oc.Sdb.Get_field_value_by_name(rec, "word_form")
			if err != nil {
				fmt.Printf("Error! %v\r\n", err)
				return result, err
			}

			word, err := oc.Sdb.Get_field_value_by_name(rec, "word")
			if err != nil {
				fmt.Printf("Error! %v\r\n", err)
				return result, err
			}
			word_property, err := oc.Sdb.Get_field_value_by_name(rec, "word_property")
			if err != nil {
				fmt.Printf("Error! %v\r\n", err)
				return result, err
			}
		...
		if len(ds) > 0 {
			for i, _ := range ds {
				if len(ds[i].FieldsValue) > 0 {
					rec, err1 := oc.Sdb.Get_fields_value_with_name(ds[i])
					if err1 != nil {
						fmt.Printf("Error %v\r\n", err1)
						return result, err1
					}
...
func FindWordCorpora2(word string, oc *OCorpora) (string, string) {
	...
	ds1, num, err1 := oc.Sdb.Find_record_index_string([]string{"word"}, word_data1)
	if err1 != nil {
		fmt.Printf("Error %v\r\n", err1)
		return "", ""
	}
